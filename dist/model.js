...
export function validateProject(p){
 ...
 if(!p.flight)throw Error('우주검증 입력이 필요합니다.');number(p.flight.months,'검증 개월',.01,120);number(p.flight.devices,'검증 소자 수',1,100000);number(p.flight.events,'관측 사건',0,1e9);if(!Number.isInteger(p.flight.devices)||!Number.isInteger(p.flight.events))throw Error('관측 소자와 사건 수는 정수여야 합니다.');
